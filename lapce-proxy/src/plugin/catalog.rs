...
use lapce_rpc::{
    RpcError,
    dap_types::{self, SetBreakpointsResponse},
...
use super::{
    ...
    loader::{load_all_volts, start_volt},
    psp::{ClonableCallback, PluginServerRpc, PluginServerRpcHandler, RpcCallback},
};
use crate::plugin::{loader::enable_volt, psp::PluginHandlerNotification};
...
impl PluginCatalog {
    ...
    pub fn handle_notification(&mut self, notification: PluginCatalogNotification) {
        ...
        match notification {
            ...
            InstallVolt(volt) => {
                tracing::warn!("InstallVolt disabled: {:?}", volt);
            }
            ReloadVolt(volt) => {
